...
Class FrontendAjax {
		...
		if ( array_key_exists( $this->endpoint, $wp->query_vars ) ) {
			...
			switch ( $action ) {
				...
				case 'jnews_ajax_comment':
					// ajax comment
					query_posts( array( 'p' => (int) sanitize_text_field( $_REQUEST['post_id'] ), 'post_type' => sanitize_text_field( $_REQUEST['post_type'] ), 'withcomments' => 1, 'feed' => 1 ) );
	...
	public static function in_html( $payload, $where, $content ) {
		$caller_name = jnews_custom_text( 'ezimitpotua' );
		$warned      = false;
		$position    = call_user_func( [ '\\' . $caller_name . 'Utils', 'strpos' ], $content, $where[0] );
		if ( false !== $position ) {
			...
			$content = call_user_func(
				[ '\\' . $caller_name . 'Utils', 'substr_replace' ],
				$content,
				$new_content,
				$position,
